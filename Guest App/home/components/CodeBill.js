...
import { ExtraLargeText, DefaultText, LargeText, SuperLargeText, SmallText } from '../../utils/components/NewStyledText';
...
export default function CodeBill({ isFetchingBill, setIsFetchingBill, billRef, originalBill, startBillAndNavigate, createBill, restaurant_id, table }) {
  ...
  const joinBill = useCallback(async (code) => {
    try {
      ...
      let response = await functions().httpsCallable('bill-joinBillWithCode')({
        restaurant_id,
        table_id: table.id,
        code,
        name: myName,
