...
import { useIsMyAccountAdmin, useIsMyAccountAnonymous, useMyID, useMyName, } from '../../utils/hooks/useUser';
...
export default function SettingsScreen({ navigation, route }) {
  ...
  const isAnonymous = useIsMyAccountAnonymous()

  const appSettings = [
    ...(!isAnonymous && !auth.currentUser?.uid) ? [{
  ...
  const [legalSettings] = useState(([
    {
      ...
      iconSize: 24,
      onTouch: () => { Linking.openURL('http://tortepay.com/privacy.html') },
    },
    {
      text: 'End-User License Agreement',
      iconName: 'business-center',
      iconSize: 24,
      onTouch: () => { Linking.openURL('http://tortepay.com/eula') },
    },
    {
    ...
    },*/
    {
      ...
      onTouch: async () => {
        let response = await MailComposer.composeAsync({
          ...
          dispatch(doAlertAdd('Cannot open email app', 'Sorry, please send an email to feedback@tortepay.com'))
        })
    ...
    },
    {
      ...
      onTouch: async () => {
        let response = await MailComposer.composeAsync({
          ...
          subject: 'Help!'
        }).catch(error => dispatch(doAlertAdd('Cannot open email app', 'Sorry, please send an email to support@tortepay.com')))
        if (response?.status === 'sent') {
          // navigation.goBack()
          // navigation.navigate('Home', { successFeedback: true }) 
        }
  ...
  const authSettings = useMemo(() => [
    {
      ...
      onTouch: () => {
        if (isAnonymous) {
          dispatch(doAlertAdd('Lose account forever?', 'You will be unable to sign back in once you sign out.', [
            {
              text: 'Yes, sign out forever',
              onPress: () => {
                dispatch(doUserEnd())
      ...
      <View style={styles.settingsContainer}>
        <View style={styles.settingsTop}>
          ...
          <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', marginRight: 40 }}>
            ...
            {!!auth.currentUser?.uid && <View style={{ marginVertical: 20, paddingLeft: 20 }}>
              <View style={{ flex: 5 }} />
              <DefaultText key='name' maxFontSizeMultiplier={1.4} >{myName || '(anonymous)'}</DefaultText>
        ...
        <ScrollView style={{}}>
          ...
          {
            !!auth.currentUser?.uid && appSettings.map((setting, index) => <SettingsRow key={index} {...setting} />)
          }
          ...
          {
            !!auth.currentUser?.uid && authSettings.map((setting, index) => <SettingsRow key={index} {...setting} />)
          }

          <View style={styles.sectionBreak} />

          {
            (isAdmin || __DEV__) && <SettingsRow
              text='Demo bill'
              iconName='test-tube'
              iconSize={24}
              IconSource={MaterialCommunityIcons}
              onTouch={() => {
                const createDemo = async (isWithUserOrder, isWithServerOrder) => {
                  try {
                    const { data: { bill } } = await httpsCallable(functions, 'demo-createDemoBill')({
                      table_id: null,
                      isWithUserOrder,
                      isWithServerOrder,
                      name: myName
                    })
                    dispatch(doAlertAdd(`Bill #${bill.bill_code} at ${bill.table.name}`, `bill_id: ${bill.id}`, [
                      {
                        text: 'Go to bill',
                        onPress: () => navigation.navigate('Link', { restaurant_id: bill.restaurant_id, bill_id: bill.id, })
                      },
                      {
                        text: 'Ok, thanks'
                      }
                    ...
                    console.log('SettingsScreen demo bill errro: ', error)
                    dispatch(doAlertAdd('Failed to create demo bill'))
                  }
                }

                dispatch(doAlertAdd('Preload items?', undefined, [
                  { text: 'User and server', onPress: () => createDemo(true, true) },
                  { text: 'User', onPress: () => createDemo(true, false) },
                  { text: 'Server', onPress: () => createDemo(false, true) },
                  { text: 'Empty', onPress: () => createDemo(false, false) },
                ], () => { }))
              }}
            />
          }
