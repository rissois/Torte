...
const MajorButton = ({ gradient, iconName, text, screen, IconSet, iconScale }) => {
  const navigation = useNavigation()
  return (
    <TouchableOpacity
    ...
    >
      ...
      >
        <IconSet
          name={iconName}
          size={Layout.window.height * BUTTON_SCALE * iconScale}
          color={Colors.white}
        />
      </LinearGradient>
      <DefaultText maxScale={1.2} center style={styles.largeButtonTitle}>{text}</DefaultText>
    </TouchableOpacity>
  )
}
