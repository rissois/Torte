...
export default function LocationsScreen({ navigation }) {
  ...
  useEffect(() => {
    console.log('hi')
    let query = firestore().collection('Restaurants')
        ...
        is_live
          ...
          >
            <QuickButton
              ...
              onPress={() => {
                dispatch(doRestaurantStart(restaurant.id))
...
const RestaurantModal = ({ restaurant: { name, days, website: { url }, description, phone, address: { line1, line2, city, state, zip_code }, cuisine, price_range }, close }) => {
  const [dayWidth, setDayWidth] = useState(null)

  return <SafeView unsafeColor={Colors.black}>
    <Header left={<BackIcon name='close' backFn={close} />} />

    <ScrollView style={{ marginHorizontal: Layout.marHor }}>
