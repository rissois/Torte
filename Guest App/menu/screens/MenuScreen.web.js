...
import { selectRestaurantIsOrderEnabled } from '../../redux/selectors/selectorsRestaurant2';
...
export default function MenuScreen({ navigation, route }) {
  ...
  const isOrderEnabledBill = useSelector(selectIsBillOrderEnabled)
  ...
  return (
    <SafeView>
      ...
      {/* No real reason requestBill until pickup */}
      {!isMenuOnly && !isFilterOpen && !isChangerOpen && <BottomButton
        backgroundColor={isOrderEnabledRestaurant && isOrderEnabledBill ? isOrderWithUser ? Colors.red : Colors.purple : Colors.darkgrey}
        onPress={() => {
          if (isMenuOnly) requestBill()
          else if (isOrderWithUser) navigation.navigate('Order')
          else navigation.navigate('Cart')
        }}
        text={isOrderWithUser ? bottomButtonWithIcon : isMenuOnly ? 'Start / join a bill' : isOrderEnabledRestaurant && isOrderEnabledBill ? `Review cart (${numberOfItemsInCart})` : 'Not accepting orders'}
        disabled={!isOrderEnabledRestaurant || !isOrderEnabledBill}
      />}
