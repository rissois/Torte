import { getFirestore, serverTimestamp, arrayUnion, arrayRemove, collection, doc, runTransaction } from 'firebase/firestore'
...
export const doTransactBillGroup = (quantity, comment, bill_group_id) => {
    return async function (dispatch, getState) {
        ...
        const groupRef = !!bill_group_id ? doc(billRef, 'BillGroups', bill_group_id) : doc(collection(billRef, 'BillGroups'))
        ...
        try {
            await runTransaction(firestore, async transaction => {
                ...
                transaction.set(billRef, {
                    cart_status: {
                        bill_group_ids: arrayUnion(groupRef.id)
                    }
                }, { merge: true })
                ...
                if (bill_order_id && getState().bill.billOrders[bill_order_id]?.wait_user_ids?.includes(myID)) {
                    transaction.set(doc(billRef, 'BillOrders', bill_order_id), {
                        timestamps: {
    ...
    return async function (dispatch, getState) {
        ...
        try {
            await runTransaction(firestore, async transaction => {
                ...
                transaction.set(billRef, {
                    cart_status: {
                        bill_group_ids: arrayRemove(bill_group_id)
                    }
                }, { merge: true })
                ...
                transaction.delete(doc(billRef, 'BillGroups', bill_group_id))
            })
        }
        catch (error) {
