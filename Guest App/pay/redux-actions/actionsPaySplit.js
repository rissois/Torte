    ...
    return async function (dispatch, getState) {
        ...
        try {
            await firestore().runTransaction(async transaction => {
                const billUserRef = billRef.collection('BillUsers').doc(myID)
                ...
                billItemSnapshots.forEach(billItemSnapshot => {
                    ...
                    if (!claimed[myID]?.length) {
                        claimed[myID] = firestore.FieldValue.delete()
                    }

                    transaction.set(billRef, {
                        bill_item_status: {
                            ordered: available.length ? firestore.FieldValue.arrayUnion(bill_item_id) : firestore.FieldValue.arrayRemove(bill_item_id),
                            claimed: Object.keys(claimed).length ? firestore.FieldValue.arrayUnion(bill_item_id) : firestore.FieldValue.arrayRemove(bill_item_id),
                        }
                    }, { merge: true })
                ...
                transaction.set(billUserRef, {
                    claim_summary: {
                        ...
                        bill_item_ids: initialNum ? firestore.FieldValue.arrayUnion(...succeeded_bill_item_ids) : firestore.FieldValue.arrayRemove(...succeeded_bill_item_ids)
                    },
                }, { merge: true })
            })
