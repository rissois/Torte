...
export default function LineItemChanges({
  ...
  currentComped, currentSize, currentFilters, currentModifiers, currentUpsells, currentQuantity, currentSubtotal, currentCustom,
  ...
  const priceBeforeAfter = useMemo(() => {
    ...
    return <BeforeAndAfter
      before={<LargeText>{centsToDollar(currentSubtotal)} each ({centsToDollar(currentTotal)} TOTAL)</LargeText>}
    ...
    />
  }, [quantity, currentQuantity, subtotal, currentSubtotal, editQuantity])

  const compedBeforeAfter = useMemo(() => {
    if (!currentComped?.percent && !currentComped?.subtotal && !comped?.percent && !comped?.subtotal) return null
    const isCompedAltered = currentComped?.percent !== comped?.percent && currentComped?.subtotal !== comped?.subtotal
    return <BeforeAndAfter
      before={currentComped?.percent ? <LargeText><Text style={[isCompedAltered && styles.strikeThrough]}>{currentComped?.percent}%</Text></LargeText> : currentComped?.subtotal ? <LargeText><Text style={[isCompedAltered && styles.strikeThrough]}>{centsToDollar(currentComped?.subtotal)}</Text></LargeText> : <NoField field='comp' />}
      after={comped?.percent ? <LargeText><Text style={[isCompedAltered && styles.bold]}>{comped?.percent}%</Text></LargeText> : comped?.subtotal ? <LargeText><Text style={[isCompedAltered && styles.strikeThrough]}>{centsToDollar(comped?.subtotal)}</Text></LargeText> : <NoField isRedBold field='comp' />}
    />
  }, [comped, currentComped])
