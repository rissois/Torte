...
import { useLineItemField, } from '../../hooks/useLineItems4';
...
import { useBillItemComped, useBillItemCustom, useBillItemFilters, useBillItemModifiers, useBillItemSize, useBillItemUpsells } from '../../utils/hooks/useBillItem';
...
const isCompedIdentical = (a = {}, b = {}) => a.is_comped === b.is_comped && a.subtotal === b.subtotal || a.percent === b.percent

export default function LineItemEditor({ bill_id, lineItemID, close, isOrder }) {
  ...
  const reference_ids = useLineItemField(bill_id, lineItemID, 'reference_ids', isOrder)
  const position = useLineItemField(bill_id, lineItemID, 'position', isOrder)
  const item_id = useLineItemField(bill_id, lineItemID, 'item_id', isOrder)
  const variant_id = useLineItemField(bill_id, lineItemID, 'variant_id', isOrder)
  const is_voided = useLineItemField(bill_id, lineItemID, 'is_voided', isOrder)

  const [pressType, setPressType] = useState(is_voided ? 'unvoid' : '')


  const name = useLineItemField(bill_id, lineItemID, 'name', isOrder)
  const currentSubtotal = useLineItemField(bill_id, lineItemID, 'subtotal', isOrder)
  const billItemIDs = useLineItemField(bill_id, lineItemID, 'bill_item_ids', isOrder)
  ...
  const currentComped = useBillItemComped(bill_id, billItemIDs?.[0])
  ...
  const isAltered = useMemo(() => {
    return !!unvoid.length
      ...
      || !isCompedIdentical(comped, currentComped)
  }, [add, remove, unvoid, size, currentSize, filters, currentFilters, modifiers, currentModifiers, upsells, currentUpsells, custom, currentCustom, comped, currentComped])
  ...
  const discard = useCallback(() => {
    ...
    setComped(currentComped)
  }, [currentSize, currentFilters, currentModifiers, currentUpsells, currentCustom, currentComped])
    ...
    <View style={{ flex: 1, position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: Colors.background }}>
      <View style={{ marginTop: 10 }}>
        <Header backFn={() => {
          ...
          {
            lineItemID ?
              <LargeText center>Edit {name}</LargeText> :
      ...
      <View style={{ marginHorizontal: Layout.marHor, flex: 1, }}>
        ...
        <LineItemChanges
          ...
          currentComped={currentComped} currentSize={currentSize} currentFilters={currentFilters} currentModifiers={currentModifiers} currentUpsells={currentUpsells} currentQuantity={billItemIDs.length} currentSubtotal={currentSubtotal}
