...
import { EditComped } from './EditComped';
import { LargeText } from '../../utils/components/NewStyledText';

export const EditLineItem = ({ item_id, variant_id, comped, setComped, size, setSize, filters, setFilters, modifiers, setModifiers, upsells, setUpsells, incompleteModifiers, setIncompleteModifiers, custom, setCustom, setEditCustomIndex, isNew }) => {
  /*
  ...
  */
  ...
  useEffect(() => {
    if (isNew) {
      ...
      setComped({ subtotal: 0, percent: 0, is_comped: false, })
      setFilters({})
      setModifiers({})
      setUpsells([])
  ...
  // Determine which option to preselect
  useEffect(() => {
  ...
  return <View style={{ flex: 1 }}>
    <LargeText>Maybe you can get away with comped up here...</LargeText>
    ...
    {
      field === 'comped' && <EditComped comped={comped} setComped={setComped} />
    }
    {
