...
export default function ItemScreen({ navigation, route }) {
  ...
  const [filterTitleWidth, setFilterTitleWidth] = useState(null)
  ...
  const [filterCanWidth, setFilterCanWidth] = useState(null)
                ...
                filtersAllowed ?
                  <View>
                    <View style={{ flexDirection: 'row', paddingHorizontal: 16, marginVertical: 12 }}>
                      <View style={{ width: filterTitleWidth, }} />
                      <View style={{ flex: 1, flexDirection: 'row', }}>
                        <View style={{ position: 'absolute', top: 0, bottom: 0, justifyContent: 'center', width: (Layout.window.width * 0.8 - filterCanWidth - filterTitleWidth - 32 - 40) * 2 / 3 + 20 }}>
                          <MainText center>Always</MainText>
                        </View>
                        <View style={{ position: 'absolute', top: 0, bottom: 0, right: 0, justifyContent: 'center', width: (Layout.window.width * 0.8 - filterCanWidth - filterTitleWidth - 32 - 40) * 2 / 3 + 20 }}>
                          <MainText center>Never</MainText>
                        </View>
                      </View>
                      <View style={{ width: filterCanWidth, }}>
                        <MainText center>Can be</MainText>
                        <MainText center>(added cost)</MainText>
                      </View>
                    </View>
                    ...
                    {Object.keys(filterTitles).map(key => {
                      return <Filter
                        ...
                        setFilterCanWidth={setFilterCanWidth}
                        setFilter={val => setFilters(prev => ({ ...prev, [key]: val }))}
                        setFilterTitleWidth={setFilterTitleWidth}
                        filterTitleWidth={filterTitleWidth}
                        onChangeText={(text) => {
                          if (!text) {
                            text = '0'
                          }
...
const Filter = ({ title, value, cost, onChangeText, setFilter, setFilterTitleWidth, filterTitleWidth, setFilterCanWidth }) => {
  ...
  return <View style={[styles.filters, {
    ...
    }} style={{ width: filterTitleWidth, }}>
      <LargeText>{title}</LargeText>
    </View>
    <View style={{ flexDirection: 'row', flex: 1, justifyContent: 'space-evenly' }}>
      <TouchableOpacity onPress={() => setFilter(true)}>
        <RadioButton on={value === true} textRight={false} />
      </TouchableOpacity>
      <TouchableOpacity onPress={() => setFilter(false)}>
        <RadioButton on={value === false} textRight={false} />
      </TouchableOpacity>
    </View>
    <View onLayout={({ nativeEvent }) => setFilterCanWidth(nativeEvent.layout.width)} style={{ flexDirection: 'row', alignItems: 'center', }}>
      <TouchableOpacity style={{ marginRight: 24 }} onPress={() => {
        setFilter(cost)
      }}>
        <RadioButton on={isNumber} textRight={false} />
      </TouchableOpacity>
      <TouchableWithoutFeedback onPress={() => {
